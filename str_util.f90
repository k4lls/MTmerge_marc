...
subroutine datenum(ts_str,ts_num)
!-------------------------------------------------------------------------------
! Get the length of the time series, total file size in bytes and number of seconds
!
! Authors:  Kevin Shearer & Marc Benoit
!    Date:  December 14, 2012
!-------------------------------------------------------------------------------
    
    implicit none
    
    integer*8, intent(out)      :: ts_num       ! int*8 numerical timestamp value
    integer                     :: d, mo, y, MM, HH, SS, ios
    integer, dimension(14)      :: daypermonth
    character(19), intent(in)   :: ts_str       ! timestamp string to be converted

    call value(ts_str(1:4), y, ios)
    call value(ts_str(6:7), mo, ios)
    call value(ts_str(9:10), d, ios)
    call value(ts_str(12:13), hh, ios)
    call value(ts_str(15:16), mm, ios)
    call value(ts_str(18:19), ss, ios)
  ...
  !  daypermonth = (/0,31,59,90,120,151,181,212,243,273,304,334/)

    ts_num=int(y*365 + ceiling(y/4.)-ceiling(y/100.)+ceiling(y/400.) + daypermonth(mo) + d)
